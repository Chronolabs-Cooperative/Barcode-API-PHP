...
use \Com\Tecnick\Barcode\Exception as ColorException;

require_once __DIR__ . DS . 'Model.php';
require_once __DIR__ . DS . 'Css.php';
require_once __DIR__ . DS . 'Web.php';
require_once __DIR__ . DS . 'Spot.php';
require_once __DIR__ . DS . 'Pdf.php';
...
abstract class Type extends \Com\Tecnick\Barcode\Type\Convert
{
    ...
    /**
     * Get Barcode as GIF Image (requires GD or Imagick library)
     */
    public function getGif()
    {
        $data = imagegif($this->getGd());
        ...
        if (strpos($_SERVER["REQUEST_URI"], "?download"))
        {
            ...
            header('Expires: Thu, 04 jan 1973 00:00:00 GMT'); // Date in the past
            header('Last-Modified: '.gmdate('D, d M Y H:i:s').' GMT');
            header('Content-Disposition: inline; filename="'.md5($data).'.gif";');
            if (empty($_SERVER['HTTP_ACCEPT_ENCODING'])) {
                // the content length may vary if the server is using compression
                header('Content-Length: '.strlen($data));
            }
    ...
    /**
     * Get Barcode as JPG Image (requires GD or Imagick library)
     */
    public function getJpg()
    {
        $data = imagejpeg($this->getGd());
        ...
        if (strpos($_SERVER["REQUEST_URI"], "?download"))
        {
            ...
            header('Expires: Thu, 04 jan 1973 00:00:00 GMT'); // Date in the past
            header('Last-Modified: '.gmdate('D, d M Y H:i:s').' GMT');
            header('Content-Disposition: inline; filename="'.md5($data).'.jpg";');
            if (empty($_SERVER['HTTP_ACCEPT_ENCODING'])) {
                // the content length may vary if the server is using compression
                header('Content-Length: '.strlen($data));
            }
